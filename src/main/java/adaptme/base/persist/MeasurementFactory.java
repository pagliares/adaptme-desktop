...
public class MeasurementFactory {
    ...
    public static List<DurationMeasurement> createIntegerDistributionForDuration(
	    IntegerDistribution integerDistribution, int size, TimeEnum scale) {
    ...
    public static List<DurationMeasurement> createRealDistributionForDuration(RealDistribution realDistribution,
	    int size, TimeEnum scale) {

	List<DurationMeasurement> list = new ArrayList<>();
	DurationMeasurement measurement;
	for (int i = 0; i < size; i++) {
	    measurement = new DurationMeasurement();
	    measurement.setValue(realDistribution.sample());
	    measurement.setScale(scale);
	    list.add(measurement);
	}
	return list;
    ...
    public static List<RoleMeasurement> createIntegerDistributionForRoleMeasurement(
	    IntegerDistribution integerDistribution, int size, TimeEnum experienceScale) {
    ...
    public static List<RoleMeasurement> createRealDistributionForRoleMeasurement(RealDistribution realDistribution,
	    int size, TimeEnum experienceScale) {
	Random random = new Random();
	List<RoleMeasurement> list = new ArrayList<>();
	RoleMeasurement measurement;
	for (int i = 0; i < size; i++) {
    ...
    public static List<WorkProductMeasurement> createIntegerDistributionForWorkProductMeasurement(
	    IntegerDistribution integerDistribution, int size) {
	...
	for (int i = 0; i < size; i++) {
	    ...
	    measurement.setQuantityScale("quantity scale"); // TODO - receber
							    // como parametro?
	    measurement.setSize(integerDistribution.sample()); // and when
							       // quantity has
							       // one
							       // distribution
							       // and size
    ...
    public static List<WorkProductMeasurement> createRealDistributionForWorkProductMeasurement(
	    RealDistribution realDistribution, int size) {
	List<WorkProductMeasurement> list = new ArrayList<>();
	WorkProductMeasurement measurement;
	for (int i = 0; i < size; i++) {
	    measurement = new WorkProductMeasurement();
	    measurement.setName("work product measurement");
	    measurement.setQuantity((int) realDistribution.sample());// TODO
								     // verify
								     // casting
	    measurement.setQuantityScale("quantity scale"); // TODO - receber
							    // como parametro?
	    measurement.setSize(realDistribution.sample()); // and when quantity
							    // has one
							    // distribution and
							    // size another? the
							    // same for the
	...
	List<DurationMeasurement> result = createIntegerDistributionForDuration(poissonDistribution, 50,
		TimeEnum.MINUTES);

	for (int i = 1; i <= result.size(); i++) {
	    System.out.println(i + " " + result.get(i - 1).getValue());
	}
	System.out.println();
	...
	List<RoleMeasurement> listRoleMeasurement = createIntegerDistributionForRoleMeasurement(
		uniformIntegerDistribution, 50, TimeEnum.DAYS);
	...
	List<WorkProductMeasurement> listWorkProductMeasurement = createIntegerDistributionForWorkProductMeasurement(
		binomialDistribution, 50);
